...
echo "  Removed mihomo/nikki/fchomo/owntone package dirs"
...
if [ -d "$SHORTCUT_FE_SRC" ]; then
  ...
  if [ -f ""/sfe_cm.c"" ]; then
    sed -i 's/tn->tcp_no_window_check)/0) \/* tcp_no_window_check removed in Linux 6.18+ *\//' ""/sfe_cm.c""
    echo ""  Patched sfe_cm.c for Linux 6.18+""
  fi

  # Add DSFE_SUPPORT_IPV6 to ccflags if not present
  if ! grep -q ""DSFE_SUPPORT_IPV6"" ""/Makefile""; then
    sed -i '/^ccflags-y/s/$/ -DSFE_SUPPORT_IPV6/' ""/Makefile""
